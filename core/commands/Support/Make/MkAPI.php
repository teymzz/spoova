...
class MkAPI extends MkBase{
    ...
    public function build() : bool{
        ...
        if(!$class) {
            ...
            Cli::textView('Syntax: '.Cli::btn('mi').' '.Cli::alert('add:api').Cli::warn('<apiName>'.Cli::textIndent(Cli::color('<extends?>', 'purple'), '1').Cli::textIndent(Cli::color('<subdir?>', 'green'), '1').Cli::danger('[-O?]', '1'), '1'), '2', "|1");                                                         
        ...
        if($overwrite && ($overwrite != '-O')){
            ...
            Cli::prompt(['y','n', '::nocase'=>true], function(CliPrompt $prompt) use($syntax){

                if($prompt->active && $prompt->invalid()) {
                    Cli::textView(Cli::emos('bullet').'Do you mean "'.Cli::warn("add:api $syntax -O").'"? [Y/N] ');
                }elseif($prompt->matches('n') || $prompt->inactive()){
        ...
        if($Filemanager->openFile(true, $routeDir)) {
            ...
            if(is_file($filePath) && !$overwrite){
                ...
                Cli::prompt(['y','n','::nocase'=>true], function(CliPrompt $prompt){
                    ...
                    if($prompt->matches(['n']) || ($prompt->inactive() && $prompt->invalid())) {
                        Cli::textView(br().Cli::alert('Notice:').' File overwrite aborted!', 0, '|2')->exit();
                    }
    ...
    private function isSubDir(string $val) {
       return ((substr($val, 0, 1) == '.') || (substr($val, 0, 1) == '\\'));
    }

    private function isMinus(string $val) {
