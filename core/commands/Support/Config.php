...
use spoova\mi\core\commands\Root\Console;
...
class Config extends Entry{
    ...
    function __construct(array $args = []){
        ...
        // ddump(CliDev::isTerminal('wt'));
        $arg = $args[0]?? false;
  
        if(empty($arg)){

            Cli::headerView('mi config:', break: 2);
            Cli::textView('This command is used for configuration.', break: 2);
            $args = 'mi config:[option] <args> args option';

            Cli::textView(Cli::emo('bullet').' Syntax:'.self::mi('config:','','','').Cli::warn('[option] <args>'), '2', break: '|2');
            
            // Cli::textPlain(Cli::warn(Cli::emo('bullet')), '2');
            Cli::infoView(' Info ', '','warn')->textView(Cli::warn('[option] => '). "[all|dbonline|dboffline|env|init|meta|usersTable|cookieField|idField]", 0, "|2");
            // Cli::textPlain(Cli::warn(Cli::emo('bullet')), '2');
            Cli::infoView(' Info ', 'use','warn')->textPlain(Cli::underline(Cli::warn('mi', '|1').Cli::danger('info config:').Cli::warn('[option]')).' for an option\'s description.', '1', "|1");
            Cli::response(false, 'program terminated because no argument was supplied.');
            return;   
        }  
        ...
        Cli::headerView('config: '.$arg, break: 1);
    ...
    public function all(){
        ...
        Cli::textView(Cli::warn("Database connection parameters (online): \"name user pass server port\""), 0, "|1");
        Cli::textView(Cli::emo("ribbon-arrow", "1|1"), '2');
        $response = Cli::prompt()->value();    
        ...
        if(!online){
            ...
            $response = strtolower(Cli::prompt()->value()); 
                ...
                if($db = $dbc->openTool($oconfigs)){
                    ...
                    if($dbname) {
                        ...
                        if($dbc->openDB($oconfigs)){
                            ...
                            if(strtolower($response) === 'y'){
                                if($db->createDB($dbname)){
                                    Cli::textView(Cli::success('database name created successfully'), break: "|2");                                    
                                }else{
                                    Cli::textView(Cli::error('database name creation failed'), break: "|2");  
                        ...
                        if(strtolower($response) === 'y'){
                            ...
                            Cli::textView(Cli::alert("Please enter your database name: "));
                            $dbname = strtolower(Cli::prompt()->value());

                            if(!trim($dbname)){
                                Cli::response(false, 'process aborted!');
                                Cli::textView(Cli::error("process aborted because no name was supplied"), 0, '|2');
                                exit;                                     
                            }
                            
                            if($db->createDB($dbname)){
                                Cli::textView(Cli::success('database created successfully'), break: "|2");                                    
                            }else{

                                Cli::response(false, 'database creation failed!');
                                // Cli::textView(Cli::error("some error occured!"), break: '|2');
                                Cli::textView(Cli::error($db->error(true)), break: '|1');
                                // Cli::textView(Cli::error("process aborted!"), 0, '|2');
                                exit;                            
                            }

                        }else{
                            Cli::textView(Cli::warn("skipping database name configuration..."));
                            Cli::break(1);
                        }
                    ...
                    Cli::clearUp(1);
                    Cli::textView(Cli::error("database connection test failed!"), break: 1);
                    return Cli::response(false, "process terminated!");
                    
                }

            }else{

                Cli::clearUp(Cli::isTerminal('linux')? 1 : 0);
                Cli::textView(Cli::warn('Notice: ').('process aborted because connection must be tested.'), break: '|1');
        ...
        if($useridTable && ($useridField || $cookie) ) {
            ...
            if($db){
                ...
                if(!$db->table_exists($useridTable)) {
                    ...
                    Cli::textView(Cli::warn("Notice: ").("table \"$useridTable\" is required by session class but does not exist in database."), 0, "1|2");
                    ...
                    $response = Cli::q($options, fn() => 
                        ...
                        [
                            'test' => fn($input) => 
                                in_array(strtolower($input->value()), $options),
                    ...
                    if(strtolower($response) === 'y') {
                        ...
                        Cli::textView(Cli::warn("skipping migration file generation..."), "0", break: 2);
        ...
        //meta option     
        Cli::textView(Cli::alert("Allow resource class to build meta tags from setup file? [Y/N] "), break: '1');        
        $response = Cli::prompt();           
        $option = $response->matches('y')?  'on' : "off";
        Cli::clearUp(1);
        $this->meta($option);
        ...
        Cli::textView(Cli::success("all configurations completed."));
        Cli::break(2);
    ...
    /**
     * Configure the icore/init app configuration file
     *
     * @return void
     */
    public function init() {
        
        self::commandTitle('config:init'.Cli::br());
        ...
        if(func_num_args() > 2){
            ...
            if($value === '--'){
                if($Filemanager->openFile()){
                    if(Init::value($key) !== false){
                        if(Init::unset($key)){
                            Cli::textView(Cli::success('configuration key removed.'), 0, "|2");
                        }else{
                            Cli::textView(Cli::error('configuration key removal failed'), 0, "|2");
                        }
                    }else{
                        Cli::textView(Cli::error('configuration key does not exist.'), 0, "|2");
                    }
                }
            }elseif(trim($value)){

                if($Filemanager->openFile()){
                    if(Init::set($key, $value)){
                        Cli::textView(Cli::success('key configuration successful'), 0, "|2");
                    }else{
                        Cli::textView(Cli::error('key configuration failed'), 0, "|2");
                    }
     ...
     * @return void
     */
    public function create_migration(array $arg) {
     ...
     * 
     * @param string|array|null $args arguments of online setup
     * @return void
     */
    public function dbonline(string|array|null $args = null){
        ...
        if(!self::$all){
            ...
            $prompt = Cli::prompt(['y', 'n', '::nocase'=> true], function(CliPrompt $prompt){

                if($prompt->matches('n')){
                    Cli::textView(Cli::alert("Notice: ").'Default offline connection unchanged.', break: '1|1')->exit();
                }

                if($prompt->maximum()){
                    Cli::textView(Cli::danger("Info: ").'Maximum number of trials reached!', break: '1|1')->exit();
                }

                if($prompt->valid()) return true;

                Cli::textView("Use the same configuration for offline environment? [Y/N] ");
     ...
     * @return void
     */
    public function dboffline(string|array|null $args = null) {
        ...
        if(!self::$all){
            ...
            Cli::text(Cli::textBuild("Use the same configuration for online environment? [Y/N] "));

            $prompt = Cli::prompt(['Y','N', '::nocase'=>true], function(CliPrompt $prompt){

                if($prompt->maximum()) {
                    Cli::errorView('maximum number of trials reached!');
                }
                
                if($prompt->valid()){
                    if($prompt->imatches('N')){
     ...
     * @return string
     */
    private function testdb_var(array $args, string $option = ''): string{
        
        $arg = $args[0] ?? '';
        $arg = trim($arg , "'");
        
        if(empty($arg)){
            Cli::textView('sets database '.Str::stripStart($option, 'db').' configuration.', 0, '|1');
            Cli::textView(Cli::emo('bullet','|1').'Syntax:'.self::mi('config:'.$option,'','','').Cli::warn('"dbname dbuser dbpass dbserver dbport dbsocket"'), 0, '1|1');
            Cli::response(false, 'program terminated because no argument was supplied.');
            return '';
        }
        
        if(count($args) > 1){
            Cli::errorView('configuration values must be in quotes.', break: '|1');
            Cli::textView(Cli::emo('bullet','|1').'Syntax:'.self::mi('config:'.$option,'','','').Cli::warn('"dbname dbuser dbpass dbserver dbport dbsocket"', 1), 0, '1|1');
            Cli::textView(Cli::emo('bullet','|1').'Note: replace empty values with a single hyphen (i.e -)', 0, '|1');
            Cli::response(false, 'program terminated because invalid syntax was supplied.');
            return '';
        }
    ...
    private function render_arguments(string $arg){
        ...
        if($counts < 5){
            Cli::textView(Cli::error("invalid count(".$counts.") of values supplied."));
            return false;
        }
    ...
    private function setup_db(string $environment, array $dbvalues, string $url) {
        ...
        $offline = self::format_config($offline ?? []);
        $online = self::format_config($online ?? []);
        ...
        Cli::textView(Cli::success($environment." setup configuration successful."), break: 1);
    }

    /** reorder database configuration parameters  */
    private function format_config(array $dbparams) : array {

        $params = [];

        $params['NAME'] = $dbparams['NAME'] ?? '';
        $params['USER'] = $dbparams['USER'] ?? '';
        $params['PASS'] = $dbparams['PASS'] ?? '';
        $params['SERVER'] = $dbparams['SERVER'] ?? '';
        $params['PORT'] = $dbparams['PORT'] ?? '';
        $params['SOCKET'] = $dbparams['SOCKET'] ?? '';
    ...
    /**
     * Handle meta tags configurations
     *
     * @param mixed $args arguments retrieved are based on the total number supplied.
     * @return void
     */
    private function meta(mixed $args) : void {
        ...
        if((count($args) < 1) || (count($args) > 1)){
            ...
            Cli::textView(self::mi('config:meta').Cli::warn('[on|off]', '1'), '2', break: 2);
        ...
        if($args[0] !== 'on' and $args[0] !== 'off'){
            ...
            Cli::textView('Syntax:'.self::mi('config:meta', '', '', '').Cli::warn('[on|off]', '1'), '2')->break(2);
        ...
        if($Filemanager = $this->get_init()){
            ...
            if($Filemanager->readFile('RESOURCE_META')){
                Cli::break();
                Cli::textView(Cli::Success('meta switched '.$args[0].' successfully'));   
                Cli::break(self::$all? 1 : 2);
                $this->complete_setup();          
            } else {
                Cli::break();
                Cli::textView(Cli::Error('meta switch '.$args[0].' failed!'));   
                Cli::break(self::$all? 1 : 2);             
            }            
        }
    ...
    private function usersTable(?string $table_name = null) {
        ...
        if($table_name === null){
            Cli::textView(Cli::emo('bullet','|1').'Syntax:'.self::mi('config:','','','').Cli::warn('userTable <tablename>'), '2', break: '1|1');
            Cli::response(false, false);
            return;
        }

        if(count($args) > 1){
            Cli::textView(Cli::error("usersTable expects exactly one argument."), break: '1|1');  
            return;
        }

        if($table_name && !preg_match('/^[A-Za-z][A-Za-z0-9_]+?$/', $table_name)){
            $table_name = trim($table_name)? ' "'.Cli::danger($table_name).'" ' : '';
            Cli::textView(Cli::error("invalid database table name{$table_name}supplied."), break: '1|1');  
            return;
        }
        ...
        if($Filemanager = $this->get_init()){
            ...
            if($Filemanager->readFile('USER_TABLE')){
                ...
                Cli::textView(Cli::success(\str_pad('user table configured as', 35, '.').Cli::warn($table_name)), break: '1');    
                $this->complete_setup();         
            } else {
                if(!$table_name){
                    Cli::textView(Cli::warn('Notice: ').('user data table name not defined'), break: '1');                
                }else{
                    Cli::textView(Cli::error('user table configuration failed'), break: '1');                
                }
            }
        }
    ...
    private function idField() {
        ...
        if(count($args) < 1){
            Cli::textView(Cli::emo('bullet','|1').'Syntax:'.self::mi('config:','','','').Cli::warn('idField <columnName>'), '2', break:'1|1');
            return;
        }

        if(count($args) > 1){
            Cli::textView(Cli::error("idField expects exactly one parameter"), break: '1|1');  
            return;
        }
        ...
        if($Filemanager = $this->get_init()){
            ...
            if($Filemanager->readFile('USER_ID_FIELDNAME')){
                Cli::textView(Cli::success(\str_pad('user id field configured as', 35, '.').Cli::warn($field_name)), break: '1');    
                // $this->complete_setup();         
            } else {
                if(!$field_name){
                    Cli::textView(Cli::warn('Notice: ').('user id field name not defined'), break: '1');             
                }else{
                    Cli::textView(Cli::error('user id field configuration failed'), break: '1');             
                }
            }
        }
    ...
    private function cookieField() {
        ...
        if(count($args) > 1){
            ...
            Cli::textView(br().Cli::error("cookieField expects exactly one parameter"), break: 2);  
        ...
        if($Filemanager = $this->get_init()){
            ...
            if($Filemanager->readFile('COOKIE_FIELDNAME')){
                Cli::textView(br().Cli::success(\str_pad('cookie field configured as', 35, '.').Cli::warn($cookie_fieldname)), break: 1); 
            } else {
                if(!$cookie_fieldname){

                    Cli::textView(Cli::warn('Notice: ').('cookie field name not defined'), break: '2|1');            
                }else{
                    Cli::textView(Cli::error('cookie field configuration failed'), break: '2|1');            
    ...
    private function watch(){
        ...
        Cli::textView(Cli::danger(Cli::emo('point-list').' config:watch'), break: 2);
        Cli::textView(Cli::notice('Please Type:'.self::mi('watch', '','', '').' instead'), break: 1); 
        Cli::response(false, false);
