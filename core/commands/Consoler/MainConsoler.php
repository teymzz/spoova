...
abstract class MainConsoler {
    ...
    final public static function validate_arguments($args) : bool|string {
        ...
        if(static::$auto_respond){
            Cli::clearUp();
            Cli::bashView(Cli::danger(Cli::emo('point-list').' '.static::$cat.strtolower(basename(to_dirslash(get_called_class()))).' '.Cli::warn(implode(' ', $args))));
        }    
        ...
        if($testOps === false){
            if(static::$auto_respond){
                ...
                if(is_array($nextOps)){
                    ...
                    $nextOps = $ops;
                    $options = array_keys(array_filter($nextOps, fn($value) => $value!==''));
                }
            ...
            if(is_array($testOps)){
                ...
                $testOps = $ops;
                $options = array_keys(array_filter($testOps, fn($value) => $value!==''));
            }
            ...
            $testOps = array_filter($ops, fn($value) => $value!=='');
