...
trait CliPassword {
    ...
    public static function password(string $placeholder = '', string $hint = '',  ?string $value = null, bool $required = false, ?int $maxlength = null, array $design = ['width'=>25, 'indent' => 0, 'shape'=>'square','textColor'=>CliForms::text_field_color,'borderColor'=>CliForms::text_field_color], ?Closure $modifier = null, ?Closure $onEnd = null){
        ...
        // Define activity for displaying a text below the input field (For testing purpose)
        $GhostFunction->toAsterisk(function($text){
        ...
        return Cli::input(function(CliKey $key) use (&$info, &$cursor, &$required, $GhostFunction,$onEnd, $modifier) {
            ...
            if($key->isExit() || $key->isEnter()){
                ...
                $charsRange = $GhostFunction->toAsterisk(mb_substr($charsText, $cursorBound, $info['x']));
                ...
                if($info['bound']>=0){
                    ...
                    if($home){
                        if(array_key_exists($cursor - 1, $info['chars'])){
                            ...
                            Cli::saveCursor();
                            Cli::textPlain(' '.Cli::color($GhostFunction->toAsterisk(mb_substr($newText, 0, $info['x'] - 1)), $textColor))->restoreCursor();
                        ...
                        if($cursor >= 0){
                            if($info['bound'] >= 0){
                                if(array_key_exists($cursor - 1, $info['chars'])){
                                    Cli::moveBack()->textPlain(' '.Cli::color($GhostFunction->toAsterisk($info['chars'][$cursor - 1]??''), $textColor))->moveBack(2);
                                }
                            }
                                if(($cursor - 1) > -1)$cursor--;
                        }
            ...
            if($xe){
                if($cursor !== count($info['chars'])){
                    ...
                    Cli::cursorView( fn()=>
                        Cli::moveStart($info['margin']+1)
                        ->textPlain(Cli::color($GhostFunction
                        ->toAsterisk($bitChar).' ', $textColor))
                        ->moveBack() 
                    );
                ...
                if($cursor < count($info['chars'])){
                    ...
                    if($bound <= ($info['x'] - 1)){
                        ...
                        Cli::textPlain(Cli::color($GhostFunction->toAsterisk($bitChar).' ', $textColor))->moveBack();
                        $cursor++;
                    }
                ...
                //get characters starting from cursor position to the end of string
                $charsRange = $GhostFunction->toAsterisk(mb_substr($fullString, $cursor+1 - $info['bound'], $info['x']));
