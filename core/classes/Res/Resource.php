...
use spoova\mi\windows\Routes\Docs\Liveserver;
 ...
 * 
 * NOTE:: Rescom is extended to {@see Rescon::class} (Resource connectors)
 */
class Resource Extends Rescom{
    ...
    private function validRequirements() : bool {
        ...
        if(!@class_exists(scheme('core\classes\Bundle\Filemanager\Filemanager', false))) {
          return false;
        }
    ...
    */
    private static function filterUrl(&$url){
        ...
        // limit of 2: an attribute value may itself contain "=>"
        $attrs = explode("=>", $url, 2);
        if(count($attrs) > 1){
          $url = $attrs[0];
          $attrs = $attrs[1];
        ...
        $url = trim($split[0]);
        if(isset($split[1])){
           // a coloned suffix names the type itself, e.g. "path/handler:::css"
           $ext = trim($split[1]);
           $colons = ":::".$ext;
        }else{
    ...
    private static function call_script(string $url, string $ext, string $attrs){
      ...
      if($ext == "css" || $ext == "js"){
        ...
        // Attribs::split() pads a trailing space; trim it so the tag matches
        // what Ress emits for the same definition
        if($ext == "css"){
          return '<link href="'.$url.'"'.rtrim($attrs).'>';
        }

        if($ext == "js"){
          return '<script src="'.$url.'"'.rtrim($attrs).'></script>';
        }
    ...
    /**
     * This is used to fetch a named file.
     * 
     * @deprecated 2.6.1
    ...
    /**
     * This is used to bind named files together into a new unique group name
     * 
     * @deprecated 2.6.1
     ...
     * @return Resource
     */
    public function bind(string $name, array $names) : Resource {
      ...
      if(isset(self::$namedFiles[$name])) {
        ...
        EInfo::view('binded name "'.$name.'" already exists for resource group!');
        return $this;
     ...
     * 
     * @deprecated 2.6.1
     * @param string $group new name of group
     * @param String[] $names names of named files
     * @return Resource
     */
     ...
     * @return void|false
     */
    public static function addFile(string|bool $name, ?string $url = null){
     ...
     *
     * @deprecated 2.6.1
     * @param array|string|null $usename as group name(s) to be exported (name(s) (array | string) of group(s) to be exported)
     * @return array
     */
    public static function export(array|string|null $usename = null) : array {
    	$values = ':lists';
    ...
    /**
      * import stored urls
      */
    private static function importer(?string $usename = null, bool $execute = true){
     ...
     *  - When used as watch option, options are [::watch|::watch-seek|::watch-poll|::lock|::console]
     * @param string|false $imports references all imported urls. 
     *  - ':values' or false: $name is not resolved, imports are not cached
     *  - ':lists' - $name is not resolved, imports are cached.
     * @return string scripts
     */
    public static function import(string|array|null $name = null, string|false &$imports = false){

      	self::$storeImports   = false;
        self::$currentImports = [];  

        //call watchFile if parameters are (::watch | ::lock) and watch is not on
        $watches = ['::watch', '::watch-seek', '::watch-poll', '::lock', '::console'];
       
        $pattern = '/::(watch(?:-seek|-poll)?(?:-\d+)?|lock|console)/';
     ...
     */
    public static function error(string|int|null $point = null){
