<?php

namespace spoova\core\classes;

use spoova\core\classes\DBRead;

class DBBot {

    function __construct($table)
    {
        $this->dbTable = $table;
        return $this;

        // DBBot::read('Daily_Records', [10,20])
        //     ->shuffle();
        // DBBot('Daily_Records')

        fromJson(DBBot::read('Daily_Records'));

        //     ->where('user = ?',['abc'])
        //     ->update(['x'=>'y']);

        //     $db->query('update Daily_records where user = ? set x = ?', [])
    }

    /**
     * Read data from database table set
     *
     * @param array $fields fields to be selected. Empty array means all fields will be selected
     * @param array|integer $limit limits of data to be returned
     * @return void
     */
    public function read(array $fields = [], $limit){
        $where = $this->where;
        return new DBRead($this->table, $fields, $limit, $where);
    }

    
    public function where() {

    }

    /**
     * Undocumented function
     *
     * @param array|string $arg1 
     *      -When argument supplied is one and $arg1 is integer, sets data limit
     *      -When argument supplied is string or array sets field to be returned
     * @param array|integer $limit limits of data to be returned
     * @return void
     */
    public function data($fields, $limits = []){
        if(empty($fields)){

            if(!is_array($fields) && !is_bool($fields)){
                $fields = (array) $fields;
            }

            array_unshift($fields, '*');

        }elseif(is_string($fields)){
            $fields = (array) $fields;
        }
        $fields = implode(',', $fields);

        $query = "select ".$fields." from ".$this->databaseTable;
        
        $dbh = User::auth()->dbh();
        $dbh->query($query)->read(...$limits);

        return $dbh->results();

    }

    public function delete(){

    }

    public function insert(){

    }

    public function sort() {

    }



}