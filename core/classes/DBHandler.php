...
trait DBInsert{
   ...
   *
   * @param string $param name of database table
   * @param array $data array of binded pair key(column) and values(values) parameters
   * @return DBhandler
   */
  public function insert_into($param, $data = null){

      $this->freeVars(true);
      $this->limit = $this->buildLimit(false);
      $this->insert_into = " insert into ".$param;
...
trait DBUpdate{
   ...
   *
   * @param int $limit1 starting limit if needed
   * @param int $limit2 stopping limit if needed
   * @return void
   */
  public function update($limit1 = null, $limit2 = null){
    ...
    if($this->limit == null){
     //This line sets limit by limit method validation
      if($this->limit($limit1, $limit2) === false){
        return false; 
      }        
    }else{
...
trait DBDelete{ 
   ...
   * @return void
   */
  public function delete($limit1 = null, $limit2 = null){
...
trait Helpers {
   ...
   *        - $db->tables()           current database tables
   *        - $db->tables(arg1)       uses arg1 as the table name
   *        - $db->tables(arg1, arg2) fetches columns from (table) arg2 inside (database) arg1. 
   * 
   * @return bool|array
   */
...
trait DBError{
  ...
  public function getError($param = null){ 
    ...
    if($param === true){
      ...
      if($param !== true and $param != null){
        $errlog = isset($errlog[$param])? $errlog[$param] : null;
      }
    }
   ...
   * @return void
   */
  public function error_exists($type = null){
    if($type == "state"){
      return $this->stateFind('error');
    }
