...
class FileManager extends Enlist{
    ...
    */
      ...
      while (!feof($reading)) {
          ...
          if($separator != null && $separator !== true){
            ...
            foreach($textkeys as $keyindex => $textkey){
              ...
              if (($lineText1 = stristr($line, $textkey.$separator)) || 
                  ...
                  if(in_array($textkey,$lineText) and array_key_exists(1,$lineText)){
                      if($keyslen === 1){
                        ...
                        $value = empty(trim($lineText[1]))? '' : trim($lineText[1]);
                        ...
                        if((substr($value, 0 , 1) === '[') and ($value[-1] === "]")){
                          ...
                          if($isArrayKey){
                            fclose($reading);
                            return $data_array[$textkey] = $value;
                          }else{
                            fclose($reading);
                            return $value;
                          }
                          $data_array[$textkey] = $value;
                        }
                        fclose($reading);
                        return $data_array;
                      }else{
     ...
     * @return FileManager
     */
    public function zipUrl(string $output_name = ''){
    ...
    /**
     * Decompress the file in the last directory
     *  
     * @param bool $del true deletes file after decompressing     
     * @param bool $strict true prevents previous errors from stopping code execution
     * @return void
     */
     ...
     */
    public function move($param1 = null, $param2 = null) : bool{
...
/*
  $FileManager = new spoova\core\classes\FileManager;

  // SAMPLE 1- create a new directory
  ...
  //SAMPLE 2 - fetch from directory

  $FileManager->setUrl('dir_path');
  $FileManager->getFolders(); // return folders in 'dir_path'
  $FileManager->getFiles();   // return files in 'dir_path'
  $FileManager->move(); // item, new directory
*/

/**
 *USING ENLIST EXTENSION
 *
 *$FileManager = new spoova\core\classes\FileManager;
 *$FileManager->source($fullpath,$selected_extensions);
 *         @param string $fullpath: full directory of item to be renamed
 *
 *$FileManager->startFrom($counter);
 *         @param int $counter: dynamic unique filename;
 *
 *$FileManager->prefix($labelName);
 * 	       @param string $labelName: static prefix name for $counter
 *
 *$FileManager->reName($extension_of_ouput); 
 ...
 * $FileManager->prefix("list"); //static name of files (prefix)
 * $FileManager->startFrom(0); //dynamic counter appended to prefix to make file unique
 * $FileManager->reName('jpg'); //rename files to jpg. if parameter is not defined, default file extension is used
 * 
 * create directory, 
 * read folders, 
 * read directories, 
